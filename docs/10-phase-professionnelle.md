...
**Cinquième correctif (bandeau figé sur le jeu de démonstration après reconnexion silencieuse)** : cause racine, finalement identifiée, d'une incohérence persistante entre le bandeau et la fenêtre "Échéances" pour un même matériel (ex. bandeau affichant une échéance à quelques jours quand la fenêtre affichait ~180 jours pour la même ligne). Au chargement de la page, le jeu de démonstration s'affiche en premier ; si la personne s'était déjà connectée lors d'une session précédente, une reconnexion silencieuse recharge ensuite les vraies données — mais le rafraîchissement visuel du bandeau n'intervenait qu'en toute fin de cette reconnexion, après plusieurs actions annexes (badge de rôle, filtres, journalisation). Si l'une d'elles échouait, tout le reste — dont ce rafraîchissement — était silencieusement abandonné : le bandeau restait alors affiché avec les échéances du **jeu de démonstration** (des dates qui ne correspondent à aucune ligne réelle du classeur) alors que le reste de l'application utilisait déjà les vraies données. Le rafraîchissement a été déplacé immédiatement après l'application des données réelles, avant toute action annexe, chacune de ces actions étant désormais isolée dans son propre bloc pour ne plus pouvoir en bloquer d'autres.
