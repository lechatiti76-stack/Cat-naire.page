...
**Quatrième correctif (deux matériels différents affichés sous le même nom)** : le bandeau et la fenêtre "Échéances" utilisent tous les deux exactement le même calcul, à partir des mêmes données — s'ils affichent des nombres de jours très différents pour ce qui semble être "le même" équipement (ex. 365 j dans un endroit, 0 jour ailleurs), la cause n'est presque jamais un bug de calcul mais **deux lignes distinctes dans l'onglet `Materiels`** portant le même nom (`Title`) mais un **N° de série différent** — chacune garde alors son propre historique de contrôles et donc sa propre échéance, sans que rien ne les distingue visuellement. Deux changements adressent ce point :
- Le N° de série apparaît maintenant sous le nom dans la fenêtre "Échéances" (et dans le bandeau), pour repérer immédiatement deux lignes homonymes.
- Si des noms en double sont détectés parmi le matériel actif, un avertissement apparaît en haut de la fenêtre "Échéances" invitant à vérifier l'onglet `Materiels` (doublon probable à supprimer ou fusionner).

Par ailleurs, la mise en correspondance entre `Controles.NumSerie` et `Materiels.NumSerie` (et, côté fiche publique par QR code, entre le paramètre `?numserie=` de l'URL et les CSV publiés) ignore maintenant la casse et les espaces superflus — un N° de série saisi à la main avec une espace ou une majuscule en trop ne rend plus silencieusement invisible le contrôle le plus récent d'un équipement.
