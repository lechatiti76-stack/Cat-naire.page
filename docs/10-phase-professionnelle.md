...
**Point de vigilance identifié en creusant cette incohérence — sans rapport avec un bug de l'application** : certains matériels peuvent n'avoir qu'une ligne `Controles` très ancienne (parfois saisie à la main lors de la mise en place initiale du classeur, avec un `ControleId` qui n'a pas le format `C…` habituel) jamais remplacée depuis par un vrai contrôle validé dans l'application. Le bandeau et la fenêtre "Échéances" affichent alors, à juste titre, cette échéance ancienne (parfois déjà dépassée) — ce n'est pas une incohérence de calcul, mais un rappel légitime qu'un contrôle réel est dû pour cet équipement. Un nouveau contrôle validé via "Nouveau contrôle" résout la situation immédiatement.
