...
**Quatrième correctif (rapprochement N° de série fragile)** : la mise en correspondance entre `Controles.NumSerie` et `Materiels.NumSerie` (et, côté fiche publique par QR code, entre le paramètre `?numserie=` de l'URL et les CSV publiés) ignore maintenant la casse et les espaces superflus — un N° de série saisi à la main avec une espace ou une majuscule en trop ne rend plus silencieusement invisible le contrôle le plus récent d'un équipement. Le N° de série apparaît aussi désormais sous le nom dans le bandeau et la fenêtre "Échéances", pour repérer facilement deux lignes `Materiels` qui porteraient le même nom par erreur (un avertissement nommant les doublons apparaît alors en haut de la fenêtre "Échéances").

**Cinquième correctif (rafraîchissement du bandeau bloqué par une action annexe)** : après une reconnexion silencieuse (rechargement de page alors que la personne s'était déjà connectée lors d'une session précédente) ou une actualisation manuelle, le rafraîchissement visuel (bandeau inclus) n'intervenait qu'en toute fin de fonction, après plusieurs actions annexes (badge de rôle, filtres, journalisation). Si l'une d'elles échouait, tout le reste — dont ce rafraîchissement — était silencieusement abandonné, laissant le bandeau affiché avec un instantané périmé. Le rafraîchissement a été déplacé immédiatement après l'application des données, avant toute action annexe, chacune de ces actions étant désormais isolée dans son propre bloc pour ne plus pouvoir en bloquer d'autres.

**Sixième correctif (cause racine réelle et définitive) : l'attribut `hidden` du bandeau n'avait aucun effet visuel.** Les cinq points ci-dessus étaient tous de vrais petits bugs, mais aucun n'expliquait entièrement l'incohérence observée : la fenêtre "Échéances" recalculait correctement à chaque ouverture, alors que le bandeau pouvait rester bloqué indéfiniment sur un très ancien affichage — y compris après plusieurs actualisations et même après que les données réelles aient radicalement changé. La cause : la règle CSS `.bandeau-flash { display: flex; }` a exactement la même spécificité que la règle par défaut du navigateur `[hidden] { display: none; }` ; à spécificité égale, c'est la règle de la feuille de style de l'application qui l'emporte sur celle du navigateur. Résultat : `els.bandeauFlash.hidden = true` (posé par le code dès qu'aucune échéance n'est proche) ne masquait **jamais** réellement le bandeau à l'écran. Or, dans ce cas précis, le code retourne sans toucher au contenu de la piste défilante — qui restait donc affiché indéfiniment avec le texte généré la dernière fois où une échéance était réellement proche, même bien après que ce ne soit plus vrai. Corrigé par une règle `.bandeau-flash[hidden] { display: none; }` (spécificité supérieure cette fois) et, par sécurité, en vidant explicitement le contenu de la piste dans ce cas.

**Point de vigilance sans rapport avec un bug de l'application** : certains matériels peuvent n'avoir qu'une ligne `Controles` très ancienne (parfois saisie à la main lors de la mise en place initiale du classeur, avec un `ControleId` qui n'a pas le format `C…` habituel) jamais remplacée depuis par un vrai contrôle validé dans l'application. Le bandeau et la fenêtre "Échéances" affichent alors, à juste titre, cette échéance ancienne (parfois déjà dépassée) — ce n'est pas une incohérence de calcul, mais un rappel légitime qu'un contrôle réel est dû pour cet équipement. Un nouveau contrôle validé via "Nouveau contrôle" résout la situation immédiatement.
