/**
 * sw.js — Service worker minimal pour rendre l'application GMAO installable
 * (PWA) et utilisable hors-ligne pour sa coquille statique, indépendamment
 * du Registre des Vérifications de Matériel (voir ../sw.js) — deux PWA
 * distinctes, deux icônes/écrans d'accueil séparés.
 *
...
self.addEventListener("fetch", (event) => {
  ...
  // Réseau EN PREMIER, cache seulement en secours (hors-ligne) — voir ../sw.js
  // pour l'explication complète de ce choix.
  event.respondWith(
    fetch(event.request)
      .then((reponseReseau) => {
        if (reponseReseau.ok) {
